...
$vid2=base64_decode(base64_decode(base64_decode($_GET['vgyjurwyshaiswlfe'])));
$ret=mysqli_query($conn,"select * from tbl_student_fees where enrollment_no =$vid2");
$cnt=1;
$result = mysqli_query($conn,"SELECT * FROM tbl_admission where enrollment_no =$vid2");
$result1=mysqli_query($conn,"select * from tbl_student_fees where enrollment_no =$vid2 ORDER BY id DESC");

$result4 = mysqli_query($conn, "SELECT SUM(amount) AS value_sum FROM tbl_student_fees WHERE enrollment_no =$vid2"); 
$r3 = mysqli_fetch_assoc($result4); 
$this_month_collection1 = $r3['value_sum'];

// $result1=mysqli_query($conn,"select * from tbl_student_fees where enrollment_no =$vid2");
$result3=mysqli_query($conn,"select MIN(pending_fee) from tbl_student_fees where enrollment_no =$vid2");
?>
